...
create policy "Usuários inserem o próprio perfil" on public.profiles for insert with check (auth.uid() = id);
...
create policy "Usuários gerenciam seus próprios módulos" on public.user_modules for all using (auth.uid() = user_id);

-- Configurações Globais
create table public.settings (
  user_id uuid references public.profiles(id) on delete cascade primary key,
  moeda text default 'R$' not null,
  fechamento_dia integer default 30 not null,
  tema text default 'dark' not null,
  updated_at timestamp with time zone default timezone('utc'::text, now()) not null
);

alter table public.settings enable row level security;
create policy "Usuários gerenciam suas configurações" on public.settings for all using (auth.uid() = user_id);
...
create policy "Usuários gerenciam suas contas financeiras" on public.finance_accounts for all using (auth.uid() = user_id);
...
create policy "Usuários gerenciam suas próprias movimentações" on public.finance_movements for all using (auth.uid() = user_id);
...
create policy "Usuários gerenciam suas contas" on public.finance_contas for all using (auth.uid() = user_id);
...
create policy "Usuários gerenciam suas metas" on public.finance_metas for all using (auth.uid() = user_id);
...
-- Índices de Unicidade de Categorias (Garante que não há duplicidade de nomes)
