...
/**
 * GET /api/v1/integrations/[origin]/status
 * Métricas detalhadas de uma integração específica.
 */
export async function GET(req: Request, { params }: Params) {
  try {
    ...
    const status = await auth.service.getStatus(auth.userId, origin as IntegrationOrigin)
    return NextResponse.json(ok(status))
  } catch (err) {
    console.error('[GET /api/v1/integrations/[origin]/status]', err)
    return NextResponse.json(fail('INTERNAL_ERROR', 'Erro ao obter status'), { status: 500 })
  }
}
