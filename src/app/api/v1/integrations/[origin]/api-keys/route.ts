...
/**
 * GET /api/v1/integrations/[origin]/api-keys
 * Lista as chaves de API de uma integração.
 */
export async function GET(req: Request, { params }: Params) {
  try {
    ...
    const keys = await auth.service.listApiKeys(auth.userId, origin as IntegrationOrigin)
    return NextResponse.json(ok(keys))
  } catch (err) {
    console.error('[GET /api/v1/integrations/[origin]/api-keys]', err)
    return NextResponse.json(fail('INTERNAL_ERROR', 'Erro ao listar chaves'), { status: 500 })
  }
}
